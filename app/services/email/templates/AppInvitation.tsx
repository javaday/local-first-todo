...
interface AppInvitationProps {
	to: string;
	from: MemberModel;
	acceptLink: string;
}

export function AppInvitation(props: AppInvitationProps) {

	const { to, from, acceptLink } = props;
	...
	const previewText = `Join ${invitedByName} on ${appTitle}`;
				...
				<Body className="mx-auto my-auto bg-gray-100/70 pt-6 font-sans text-gray-900 antialiased">
					<Container className="mx-auto my-[40px] rounded bg-white">
						<Heading className="text-black text-[24px] font-normal text-center p-0 my-[30px] mx-0">
							Your <strong>{appTitle}</strong> Invitation
						</Heading>
						...
						<Text className="text-black text-[20px] leading-[24px] mt-[20px]">
							...
							) is inviting you to join <strong>{appTitle}</strong>.
						</Text>
						<Section>
							<Row>
